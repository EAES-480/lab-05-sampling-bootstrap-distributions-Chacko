...
est <- samp %>%
  summarise(across(all_of(___), ~mean(.x, na.rm = ___))) %>%
...
relerr <- est %>%
  ...
  mutate(rel_error_pct = 100 * abs(___ - ___) / abs(___)) %>%
...
n_grid <- c(___, ___, ___, ___)   # e.g., c(25, 100, 300, 1000)
reps <- ___                        # e.g., 500 or 1000

target_var <- "___"                # e.g., "fch4" or "fc"
...
for (i in seq_along(n_grid)) {
  ...
  for (j in seq_len(reps)) {
    ...
    relerrs[j] <- 100 * abs(pop_mean_target - ___) / abs(pop_mean_target)
  }
...
tibble(mean_est = means) %>%
  ...
  geom_histogram(bins = ___, alpha = 0.85) +
...
boot_means <- replicate(
  ___,
  mean(sample(samp$x, size = ___, replace = TRUE))
)
...
swc_samp_random <- sample(___, size = n_samp, replace = FALSE)

# 2) Bootstrap the mean (resample WITH replacement from the sample)
boot_means_random <- replicate(
  B,
  mean(sample(swc_samp_random, size = ___, replace = TRUE))
)

# 3) Summarize
tibble(
  pop_mean = pop_mean_swc,
  sample_mean = mean(___),
  boot_sd = sd(___)
)
```
...
swc_samp_summer <- sample(___, size = n_samp, replace = FALSE)

# 2) Bootstrap the mean from that sample
boot_means_summer <- ___(
  B,
  mean(sample(___, size = n_samp, replace = TRUE))
)
...
quantile(boot_means_random, probs = c(___, ___))
```
