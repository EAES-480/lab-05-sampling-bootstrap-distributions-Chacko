...
> *Gpp has the largest population SD. It is ahead of ta, but not by much. The fact that gpp has the highest SD indicates that there was a lot of variability when creating that dataset.*
...
> *The variable with the largest relative error at my chosen n_samp was the fc variable. Variables with higher variability tend to produce less accurate estimates when the sample size is small. When values fluctuate widely, a small sample may not capture the full range of the population. As a result, the sample mean may differ more from the true population mean.*
...
summary_relerr <- sim_relerr %>%
  group_by(n) %>%
  summarise(
    median_rel_error = median(rel_error_pct),
...
p_relerr <- sim_relerr %>%
  ...
  labs(
    x = "Sample size (n)",
    y = "Relative error (%)",
    title = "Relative error decreases as sample size increases"
  )
p_relerr
```
...
> *The improvement from 25 to 100 is larger than the improvement from 300 to 1000. This is because the largest gains in accuracy happen when moving from very small samples to moderate samples. when the sample size increases, the estimates are already closer to the population mean, so additional observations produce smaller improvements. *
...
> *se_theory and se_simulated were very similar. This means that the formulas used for each made good approximations. increasing the n_samp would decrease the erroe since there are more samples, this follows the same logic as the previous promt. Increasing the reps does not decrease the error, but improves the accuracy of the simulation.*
...
> *The bootstrap distribution looks similar to the sampling distribution of the mean. Both show how the sample mean would vary across repeated samples. The bootstrap method attempts to approximate the sampling distribution using only the observed sample. This approach is especially useful in EAES when collecting many independent samples is difficult or expensive in field studies.*
...
> *Yes, the bootstrap distribution is centered near the population mean because the sample was randomly drawn from the population. The bootstrap sd represents the estimated standard error of the sample mean. It describes how much the estimated mean would vary if the sampling process were repeated many times.*
...
> *The bootstrap distribution may appear narrow and confident even if it is centered far from the population mean. This happens because the bootstrap measures precision, not accuracy. Precision describes how consistent the estimates are, while accuracy describes how close they are to the true value. In this case, summer-only sampling introduces sampling bias because the observations do not represent the entire year.*
...
> *The random and stratified samples are closer to the population mean than the summer only sample. The summer only distribution may look precise but is biased because it represents only one season. The stratified sample is usually the most accurate for estimating the annual mean because it includes observations from all months. This demonstrates that bootstrap methods measure uncertainty within the sample but cannot correct for biased sampling designs.*
...
# Part III — Estimating 95% Confidence Intervals with Bootstrap

In this section you will use the bootstrap distributions you learned to create **95% confidence intervals**.

Recall:

A 95% bootstrap confidence interval can be estimated using the **2.5% and 97.5% quantiles** of the bootstrap distribution.

## Exercise 1 — 95% CI for the Mean (SWC)

Using the **random SWC sample from earlier**, compute a **95% bootstrap confidence interval for the mean**.

```{r ex1_boot_ci_mean, echo=TRUE, eval=FALSE}
# GOAL: compute the 2.5% and 97.5% quantiles of the bootstrap distribution

quantile(boot_means_random, probs = c(0.025, 0.975))
```

**Prompt:**

Does the confidence interval contain the **true population mean (`pop_mean_swc`)**?  
What does this tell you about the uncertainty of the estimate?

*Yes, the confidence interval contains the true population mean. This suggests that the bootstrap method provided a reasonable estimate of uncertainty around the sample mean. It indicates that the observed sample likely represents the population fairly well.*
---

## Exercise 2 — Visualizing the Confidence Interval

Modify the bootstrap histogram from Part II so that the **95% CI is shown on the plot**.

```{r ex2_plot_ci, echo=TRUE, eval=FALSE}
boot_ci <- quantile(boot_means_random, probs = c(0.025, 0.975))

tibble(boot_mean = boot_means_random) %>%
  ggplot(aes(x = boot_mean)) +
  geom_histogram(bins = 50, alpha = 0.8) +
  geom_vline(xintercept = boot_ci, color = "red", linewidth = 1.1) +
  geom_vline(xintercept = mean(swc_samp_random), linetype = "dashed") +
  theme_classic(base_size = 18)
```

**Prompt:**

Explain what the **red vertical lines** represent.

The vertical red lines represent the 2.5 and 97.5 percentiles. Between them lies the 95%, and on the left and right is 2.5% on each side.

## Exercise 3 — Confidence Intervals for a Linear Model
...
```{r ex3_fit_model, echo=TRUE, eval=FALSE}
response_var <- "swc"
predictor_var <- "ta"

model <- lm(df[[response_var]] ~ df[[predictor_var]])
...
quantile(boot_coef[,1], probs = c(0.025, 0.975))  # intercept CI
quantile(boot_coef[,2], probs = c(0.025, 0.975))  # slope CI
```

**Prompt:**

Interpret the slope confidence interval.

- Does it include zero?
- What does that imply about the relationship between the predictor and response variable?

*The slope confidence interval represents the range of plausible values for the relationship between the predictor and response variable. If the interval does not include zero, it suggests there is a statistically meaningful relationship between the variables. If the interval does include zero, it indicates that the data do not provide strong evidence of a relationship.*
---
